...
public class SetsEntityGravityProcedure {
			...
			for (Entity entityiterator : EntityArgument.getEntities(arguments, "player")) {
				{
					...
					entityiterator.getCapability(TechnologyModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
						capability.entity_gravity = _setval;
						capability.syncPlayerVariables(entityiterator);
					});
				}
				if (entity instanceof Player _player && !_player.level.isClientSide())
					_player.displayClientMessage(new TextComponent(("\u4EE5\u5C06\u5B9E\u4F53" + entityiterator + "\u7684\u91CD\u529B\u8BBE\u7F6E\u4E3A:\"" + DoubleArgumentType.getDouble(arguments, "order") + "\"")), (false));
			}
