...
import net.wei_bai.technology.client.renderer.GoodKarmaPacket;

import java.util.List;
import java.util.Map;

import static net.wei_bai.technology.init.TechnologyModEntities.GOOD_KARMA_ENTITY;

public class GoodKarmaEntity extends Entity {
    ...
    public GoodKarmaEntity(Level p_20776_, double p_20777_, double p_20778_, double p_20779_, int p_20780_) {
        this(GOOD_KARMA_ENTITY.get(), p_20776_);
        this.setPos(p_20777_, p_20778_, p_20779_);
        this.setYRot((float)(this.random.nextDouble() * 360.0D));
        this.setDeltaMovement((this.random.nextDouble() * (double)0.2F - (double)0.1F) * 2.0D,
                this.random.nextDouble() * 0.2D * 2.0D, (this.random.nextDouble() * (double)0.2F - (double)0.1F) * 2.0D);
        this.value = p_20780_;
    }

    public GoodKarmaEntity(EntityType<? extends GoodKarmaEntity> p_20773_, Level p_20774_) {
    ...
    private void scanForEntities() {
        ...
        if (this.level instanceof ServerLevel) {
            for(GoodKarmaEntity experienceorb : this.level.getEntities(EntityTypeTest.forClass(GoodKarmaEntity.class), this.getBoundingBox().inflate(0.5D), this::canMerge)) {
    ...
    public static void award(ServerLevel p_147083_, Vec3 p_147084_, int p_147085_) {
        while(p_147085_ > 0) {
            ...
            if (!tryMergeToExisting(p_147083_, p_147084_, i)) {
                p_147083_.addFreshEntity(new GoodKarmaEntity(p_147083_, p_147084_.x(), p_147084_.y(), p_147084_.z(), i));
            }
        }
    ...
    private static boolean tryMergeToExisting(ServerLevel p_147097_, Vec3 p_147098_, int p_147099_) {
        ...
        List<GoodKarmaEntity> list = p_147097_.getEntities(EntityTypeTest.forClass(GoodKarmaEntity.class), aabb, (p_147081_) -> {
            return canMerge(p_147081_, i, p_147099_);
        });
        if (!list.isEmpty()) {
            GoodKarmaEntity experienceorb = list.get(0);
            ++experienceorb.count;
            experienceorb.age = 0;
            return true;
        } else {
    ...
    private boolean canMerge(GoodKarmaEntity p_147087_) {
        return p_147087_ != this && canMerge(p_147087_, this.getId(), this.value);
    }

    private static boolean canMerge(GoodKarmaEntity p_147089_, int p_147090_, int p_147091_) {
        return !p_147089_.isRemoved() && (p_147089_.getId() - p_147090_) % 40 == 0 && p_147089_.value == p_147091_;
    }

    private void merge(GoodKarmaEntity p_147101_) {
    ...
    public void playerTouch(Player p_20792_) {
        if (!this.level.isClientSide) {
            if (p_20792_.takeXpDelay == 0) {
                if (MinecraftForge.EVENT_BUS.post(new GoodKarmaEvent.PickupXp(p_20792_, this))) return;
    ...
    public Packet<?> getAddEntityPacket() {
        return new GoodKarmaPacket(this);
    }
