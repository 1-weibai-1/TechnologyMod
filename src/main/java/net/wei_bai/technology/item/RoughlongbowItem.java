...
public class RoughlongbowItem extends BowItem {




	public RoughlongbowItem() {
		super(new Properties().tab(TechnologyModTabs.TAB_TECHNOLOGYTAB).durability(1024));
	}
			...
			if (!itemstack.isEmpty() || flag) {
				...
				if (!((double)f < 0.1D)) {
					...
					if (!p_40668_.isClientSide) {
						...
						Object entityiterator= p_40669_;
						int aacs = (int) ((LivingEntity) entityiterator).getAttribute(Attributes.ATTACK_DAMAGE).getBaseValue();
						int ds= (int) (aacs * 0.2);
						abstractarrow.setBaseDamage((abstractarrow.getBaseDamage())+ds);
						abstractarrow = customArrow(abstractarrow);
						abstractarrow.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, f * 3.0F, 0.0F);

						int base_dag = (int) abstractarrow.getBaseDamage();
						if (f == 1.0F) {
