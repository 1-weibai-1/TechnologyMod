...
import net.wei_bai.technology.item.inventory.ChuangzaoInventoryCapability;
...
public class ChuangzaoItem extends Item {
	public ChuangzaoItem() {
		super(new Item.Properties().tab(TechnologyModTabs.TAB_TECHNOLOGYTAB).stacksTo(1).fireResistant().rarity(Rarity.COMMON));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openGui(serverPlayer, new MenuProvider() {
				...
				public Component getDisplayName() {
					return new TextComponent("Chuangzao");
				}
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new GetabilityguiMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag compound) {
		return new ChuangzaoInventoryCapability();
	}
